...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектОбработки.Инициализация(ЭтотОбъект);
...
Процедура Команда_ПараметрыЗаполнитьИзЗапроса(Команда)
	...
	CтОшибка = ПараметрыЗаполнитьИзЗапросаНаСервере();
	
	Если ЗначениеЗаполнено(CтОшибка) Тогда
		
		ПоказатьПредупреждениеКонсоли(CтОшибка.ОписаниеОшибки);
		ТекущийЭлемент = Элементы.ТекстЗапроса;
		
		Если ЗначениеЗаполнено(CтОшибка.Строка) Тогда
			Элементы.ТекстЗапроса.УстановитьГраницыВыделения(CтОшибка.Строка, CтОшибка.Колонка, CтОшибка.Строка, CтОшибка.Колонка);
		КонецЕсли;
...
Процедура Команда_ЗаполнитьИзXML(Команда)
	...
	CтрОшибка = ЗаполнитьИзXMLНаСервере();
	Если ЗначениеЗаполнено(CтрОшибка) Тогда
		ПоказатьПредупреждениеКонсоли(CтрОшибка);
	КонецЕсли;
...
Процедура Команда_ФорматироватьТекстЗапроса(Команда)
	...
	CтрТекстЗапроса = ТекстЗапроса.ПолучитьТекст();
	КомментарииЗапроса_СохранитьДанныеИсходногоЗапроса(CтрТекстЗапроса);
	Результат = ФорматироватьТекстЗапросаНаСервере(CтрТекстЗапроса);
...
Процедура Команда_ПолучитьКодДляПерехватаВОтладчике(Команда)
	...
	Если Объект.РежимЗапускаВнешняяОбработка Тогда
		...
		CтрИмяФайлаОбработкиНаСервере = ПолучитьИмяФайлаОбработкиНаСервере();

		CтрКод = СтрШаблон("ВнешниеОбработки.Создать(""%1"", Ложь).СохранитьЗапрос(%2, Запрос)", CтрИмяФайлаОбработкиНаСервере, Формат(Объект.СеансИД, "ЧГ=0"));
		
	Иначе
		
		CтрКод = СтрШаблон("Обработки.%1.Создать().СохранитьЗапрос(%2, Запрос)", Объект.ИмяОбработки, Формат(Объект.СеансИД, "ЧГ=0"));
		
	КонецЕсли;
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Объект", Объект);
	ПараметрыОткрытия.Вставить("Заголовок", "Код для перехвата запроса в отладчике");
	ПараметрыОткрытия.Вставить("КодДляКопирования", CтрКод);
	ПараметрыОткрытия.Вставить("Информация", "Для перехвата запроса в отладчике скопируйте и выполните по Shift+F9 указанный код.
	...
	ОткрытьФорму(ПолноеИмяФормы("Информация"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Команда_ДобавитьПерехваченныеЗапросы(Команда)
	...
	Для Каждого стрФайл Из МаФайлыЗапросов Цикл
		Состояние("Загрузка перехваченного запроса: " + СчФайлов + " из " + МаФайлыЗапросов.Количество(), (СчФайлов - 1) * 100 / МаФайлыЗапросов.Количество(), СтрПояснение);
...
Процедура Команда_РезультатВТабличныйДокумент(Команда)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Объект", Объект);
	ПараметрыОткрытия.Вставить("АдресРезультатаЗапроса", АдресРезультатаЗапроса);
	ПараметрыОткрытия.Вставить("РезультатВПакете", РезультатВПакете);
	ПараметрыОткрытия.Вставить("ИмяЗапроса", Параметры.ИмяЗапросаРезультата);
	ПараметрыОткрытия.Вставить("ВидРезультата", ВидРезультата);
	
	ФормаТабличногоДокумента = ОткрытьФорму(ПолноеИмяФормы("ФормаТабличногоДокумента"), ПараметрыОткрытия, ЭтотОбъект, Ложь);
...
Процедура Команда_ВставитьПредопределенноеЗначение(Команда)
	...
	ПараметрыОповещения.Вставить("НачальнаяСтрока", НачальнаяСтрока);
	ПараметрыОповещения.Вставить("НачальнаяКолонка", НачальнаяКолонка);
	ПараметрыОповещения.Вставить("КонечнаяСтрока", КонечнаяСтрока);
	ПараметрыОповещения.Вставить("КонечнаяКолонка", КонечнаяКолонка);

	ОписаниеОповещенияОЗакрытииОткрываемойФормы = Новый ОписаниеОповещения("ОкончаниеВыбораПредопределенного", ЭтотОбъект, ПараметрыОповещения);
	
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("Объект", Объект);
	ПараметрыОткрытия.Вставить("ДанныеФормы", ДанныеФормыВыборПредопределенного);
	ПараметрыОткрытия.Вставить("ТекстЗапроса", ТекстЗапроса.ПолучитьТекст());
	ПараметрыОткрытия.Вставить("НачальнаяСтрока", НачальнаяСтрока);
	ПараметрыОткрытия.Вставить("НачальнаяКолонка", НачальнаяКолонка);
	ПараметрыОткрытия.Вставить("КонечнаяСтрока", КонечнаяСтрока);
	ПараметрыОткрытия.Вставить("КонечнаяКолонка", КонечнаяКолонка);
		
	ОткрытьФорму(ПолноеИмяФормы("ВыборПредопределенного"), ПараметрыОткрытия, ЭтотОбъект, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Команда_РезультатВПараметр(Команда)
	...
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("Объект", Объект);
	ПараметрыОткрытия.Вставить("ТипЗначения", ТзТаблица);
	ПараметрыОткрытия.Вставить("ТипКонтейнера", 3);
	ПараметрыОткрытия.Вставить("Имя", Параметры.ИмяЗапросаРезультата);
	ПараметрыОткрытия.Вставить("ВЗапросРазрешено", Ложь);
	ПараметрыОткрытия.Вставить("ВПараметр", Истина);
...
Процедура Команда_СправкаАлгоритм(Команда)
	...
	ОткрытьФорму(ПолноеИмяФормы("Справка"), ПараметрыОткрытия, ЭтотОбъект);
КонецПроцедуры
...
Процедура Команда_ПолучитьКодСПараметрами(Команда)
	...
	ПараметрыОткрытия = Новый Структура();
	ПараметрыОткрытия.Вставить("Объект", Объект);
	ПараметрыОткрытия.Вставить("ИмяЗапроса", ИмяЗапроса);
	ПараметрыОткрытия.Вставить("ТекстЗапроса", ТекстЗапроса.ПолучитьТекст());
	ПараметрыОткрытия.Вставить("ПараметрыЗапроса", ПараметрыЗапроса_ПолучитьКакСтроку());
	ПараметрыОткрытия.Вставить("Заголовок", "Код для выполнения запроса на встроенном языке 1С");
	ПараметрыОткрытия.Вставить("Содержание", "");

	ОткрытьФорму(ПолноеИмяФормы("ФормаКода"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , ,
...
Процедура Команда_ПланЗапроса(Команда)
	...
	Форма = ОткрытьФорму(ПолноеИмяФормы("ФормаПланаЗапроса"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработатьИзменениеИмениПараметра(НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	МтрИмяПараметра = Элементы.ПараметрыЗапроса.ТекущиеДанные.Имя;
	Если ЗначениеЗаполнено(МтрИмяПараметра) И МтрИмяПараметра = СтароеЗначениеИмяПараметра Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ИмяКорректно(МтрИмяПараметра) Тогда
	...
	МаСтрокиИмени = ПараметрыЗапроса.НайтиСтроки(Новый Структура("Имя", МтрИмяПараметра));
	...
	Если НЕ НоваяСтрока И ЗначениеЗаполнено(СтароеЗначениеИмяПараметра) Тогда
		...
		Если ЕстьПараметр(СтрТекстЗапроса, "&" + СтароеЗначениеИмяПараметра) Тогда
			ДополнительныеПараметры = Новый Структура("СтароеЗначениеИмяПараметра, ИмяПараметра", СтароеЗначениеИмяПараметра, МтрИмяПараметра);
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ИзменитьИмяПараметраВТекстеЗапроса", ЭтотОбъект, ДополнительныеПараметры),
...
Процедура ПакетЗапросовПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ПрочитатьСтруктуруЗапроса(ТекущиеДанные.ТекстЗапроса);		
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПрочитатьСтруктуруЗапроса(ТекстЗапроса)
	
	Перем ТекущаяСхемаЗапроса;
	Попытка
		ДеревоОбъект = Новый ДеревоЗначений;

		ДеревоОбъект.Колонки.Добавить("Структура", Новый ОписаниеТипов("Строка"));
		ДеревоОбъект.Колонки.Добавить("Тип", Новый ОписаниеТипов("Строка"));
		ТекущаяСхемаЗапроса = Новый СхемаЗапроса;
		ТекущаяСхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
		Для Каждого ИзвлекаемыйПакетаЗапросов Из ТекущаяСхемаЗапроса.ПакетЗапросов Цикл
			ПредставлениеТаблицыЗапроса = ИзвлекаемыйПакетаЗапросов.Представление();
			СтрокаУ1 = ДеревоОбъект.Строки.Добавить();
			СтрокаУ1.Структура = ПредставлениеТаблицыЗапроса;
			
			
			Если ИзвлекаемыйПакетаЗапросов.Колонки.Количество() <> 0 Тогда
				СтрокаУ2 = СтрокаУ1.Строки.Добавить();
				СтрокаУ2.Структура = "Колонки";
				Для Каждого ЭлементКолонки Из ИзвлекаемыйПакетаЗапросов.Колонки Цикл
					СтрокаУ3 = СтрокаУ2.Строки.Добавить();
					СтрокаУ3.Структура 	= ЗаполнитьЗначениеДерева(ЭлементКолонки, "Псевдоним");
					СтрокаУ3.Тип		= ЗаполнитьЗначениеДерева(ЭлементКолонки, "ТипЗначения");
				КонецЦикла;
			КонецЕсли;
			
			Если ИзвлекаемыйПакетаЗапросов.Операторы.Количество() <> 0 Тогда
				Если ИзвлекаемыйПакетаЗапросов.Операторы[0].Отбор.Количество() <> 0 Тогда
					СтрокаУ2 = СтрокаУ1.Строки.Добавить();
					СтрокаУ2.Структура = "Условия";
					Для Каждого ЭлементОператоры Из ИзвлекаемыйПакетаЗапросов.Операторы Цикл
						Для Каждого ЭлементаОтбор Из ЭлементОператоры.Отбор Цикл
							СтрокаУ3 = СтрокаУ2.Строки.Добавить();
							СтрокаУ3.Структура 	= ЭлементаОтбор;
						КонецЦикла;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			Если ИзвлекаемыйПакетаЗапросов.Операторы.Количество() <> 0 Тогда
				Если ИзвлекаемыйПакетаЗапросов.Операторы[0].Источники.Количество() <> 0 Тогда
					СтрокаУ2 = СтрокаУ1.Строки.Добавить();
					СтрокаУ2.Структура = "Связи";
					Для Каждого ЭлементОператоры Из ИзвлекаемыйПакетаЗапросов.Операторы Цикл
						Для Каждого ЭлементИсточники Из ЭлементОператоры.Источники Цикл
							СтрокаУ3 = СтрокаУ2.Строки.Добавить();
							СтрокаУ3.Структура 	= ЭлементИсточники.Источник.Псевдоним;
						КонецЦикла;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
...
Функция ЗаполнитьЗначениеДерева(ЭлементКолонки, ЗначениеЭлемента)
	Попытка
		Возврат ЭлементКолонки[ЗначениеЭлемента];
	Исключение
...
Процедура ПараметрыЗапросаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ТипКонтейнера > 0 Тогда
		...
		ОписаниеОповещенияОЗакрытииОткрываемойФормы = Новый ОписаниеОповещения("ОкончаниеРедактированияСтроки", ЭтотОбъект, ПараметрыОповещения);
		...
		ОткрытьФорму(ПолноеИмяФормы(ИмяФормыРедактирования), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	ИначеЕсли ТипЗнч(ТекущиеДанные.Контейнер) = Тип("Структура") Тогда
		...
		Если ТекущиеДанные.Контейнер.Тип = "МоментВремени" ИЛИ ТекущиеДанные.Контейнер.Тип = "Граница" Тогда
			...
			ОткрытьФорму(ПолноеИмяФормы("РедактированиеГраницыМомента"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		ИначеЕсли ТекущиеДанные.Контейнер.Тип = "Тип" Тогда
			...
			ОткрытьФорму(ПолноеИмяФормы("РедактированиеТипа"), ПараметрыОткрытия, ЭтотОбъект, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	...
	Иначе
		Если ТипЗнч(ТекущиеДанные.Значение) = Тип("УникальныйИдентификатор") Тогда
			...
			ОткрытьФорму(ПолноеИмяФормы("РедактированиеУникальногоИдентификатора"), ПараметрыОткрытия, ЭтотОбъект, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВременныеТаблицыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму(ПолноеИмяФормы("РедактированиеТаблицы"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура РезультатЗапросаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если РезультатЗапросаКолонкиКонтейнера.Свойство(ИмяКолонки) Тогда
		...
		Контейнер = ЭтотОбъект[Элемент.Имя].НайтиПоИдентификатору(Элемент.ТекущаяСтрока)[ИмяКолонки + "_31415926Контейнер"];
		...
		Если Контейнер.Тип = "ТаблицаЗначений" Тогда
			...
			ОткрытьФорму(ПолноеИмяФормы("РедактированиеТаблицы"), ПараметрыОткрытия, ЭтотОбъект, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПриОткрытииПродолжение(ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДополнительныеПараметры) Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьПолучениеРабочегоКаталогаДанныхПользователя", ЭтотОбъект, ДополнительныеПараметры);
	...
	ИначеЕсли ДополнительныеПараметры.ТочкаПродолжения = "ПослеПолученияРабочегоКаталога" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьПроверкуСуществования", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ЗагрузитьПакетЗапросовПослеВопроса(РезультатВопроса, ДополнительныеПараметры)
	...
	ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("Завершить_ПослеУдаления", ЭтотОбъект, ДополнительныеПараметры);
...
#Область ПрограммноеСозданиеФормы

&НаСервере
Процедура СоздатьФормуДервоЗапроса()
	Попытка
		Если Элементы.Найти("Дерево") = Неопределено Тогда
			...
			ЭлементДерево = Элементы.Добавить("Дерево", Тип("ТаблицаФормы"), Элементы.ГруппаДеревоЗапроса);
			...
			Для Каждого Колонка Из ДеревоОбъект.Колонки Цикл
				НовыйЭлемент = Элементы.Вставить(Колонка.Имя, Тип("ПолеФормы"), ЭлементДерево);
...
#Иначе
	...
	ИзвлечьРедактируемыйЗапрос();
	КонструкторЗапроса.Показать(Новый ОписаниеОповещения("Команда_КонструкторЗапроса_ОповещениеЗакрытияКонструктора", ЭтотОбъект, ТекущийЗапрос));
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	БылСкрыт = Ложь;
	Если НЕ Элементы.ГруппаСписокЗапросов.Видимость Тогда
		Элементы.ГруппаСписокЗапросов.Видимость = Истина;
		БылСкрыт = Истина;
	КонецЕсли;
	...
	Иначе
		...
		Если Элементы.ГруппаСписокЗапросов.Видимость Тогда
	...
	Параметры.ИмяЗапросаРезультата = Элементы.ПакетЗапросов.ТекущиеДанные.Имя;

	ОбновитьСостояниеЭлементовФормыАлгоритма();
	Если БылСкрыт Тогда
		Элементы.ГруппаСписокЗапросов.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПакетЗапросовСохранитьКак(Контекст)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПакетЗапросовСохранитьКак_ПослеВыбораФайла", ЭтотОбъект, Контекст);
	Диалог.Показать(ОписаниеОповещения);
...
Процедура ПакетЗапросовСохранитьКак_ПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьСохранениеФайла", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ЗакончитьСохранениеФайла(Результат, ДополнительныеПараметры) Экспорт
	ОписаниеОповещение = Новый ОписаниеОповещения(ДополнительныеПараметры.ПроцедураОповещенияЗавершенияСохранения, ЭтотОбъект, ДополнительныеПараметры);
...
Процедура СохранитьПакетЗапросов(Контекст)
	...
	Если НЕ ЗначениеЗаполнено(ИмяФайлаЗапросов) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьПакетЗапросовПродолжение", ЭтотОбъект, Контекст);
		Автосохранить(ОписаниеОповещения);
		Возврат;
	КонецЕсли;
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗакончитьСохранениеФайла", ЭтотОбъект, Контекст);
	Автосохранить(ОписаниеОповещения);
...
Функция ПолучитьИмяФайлаОбработкиНаСервере()
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Объект.ИмяФайлаОбработки) Тогда
			Адрес = "";
			НачатьПомещениеФайла(Новый ОписаниеОповещения("ПоместитьОбработкуНаСерверПродолжение", ЭтотОбъект), Адрес, Объект.ИмяФайлаОбработки, Ложь);
		КонецЕсли;
...
Функция ИмяКорректно(ПроверяемоеИмя)
	...
	Попытка
		//@skip-check module-unused-local-variable
		СтПроверяемоеИмя = Новый Структура(ПроверяемоеИмя);
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Функция ИзвлечьРезультатВДанныеФормы(РезультатВПакете)
	...
	Если Число(РезультатВПакете) <= 0 Тогда
		//@skip-warning
		РеквизитФормыВЗначение("Объект").СоздатьРеквизитыТаблицыПоКолонкам(ЭтотОбъект, "РезультатЗапроса", Неопределено);
		СтруктураЗаписиРезультата_ЗаполнитьСтруктуруЗаписи();
		Возврат 0;
	КонецЕсли;
	...
	Обработка.СоздатьРеквизитыТаблицыПоКолонкам(ЭтотОбъект, ИмяРеквизитаРезультата,
...
Функция СохранитьСВопросом(ДополнительныеПараметры)
	...
	Если Модифицированность Тогда
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ПослеВопросаСохранения", ЭтотОбъект, ДополнительныеПараметры),
...
Процедура ЗавершениеПослеВопроса(РезультатВопроса, ДополнительныеПараметры)
	...
	ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		Если ЗначениеЗаполнено(ИмяФайлаЗапросов) Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеПослеУдаления", ЭтотОбъект);
			НачатьУдалениеФайлов(ОписаниеОповещения, ПолучитьИмяФайлаАвтосохранения(ИмяФайлаЗапросов));
		Иначе
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеПослеУдаления", ЭтотОбъект);
			НачатьУдалениеФайлов(ОписаниеОповещения, ИмяФайлаАвтосохраненияСостояния);
...
Процедура ОбновитьСостояниеЭлементовФормыАлгоритма()
	...
	Если НЕ ЗначениеЗаполнено(ФоновоеЗаданиеИдентификатор) Тогда
		...
		Если Элементы.ПакетЗапросов.ТекущиеДанные <> Неопределено И Параметры.ИмяЗапросаРезультата = Элементы.ПакетЗапросов.ТекущиеДанные.Имя Тогда
...
Процедура ПоместитьРедактируемыйЗапрос()
	...
	Если РедактируемыйЗапрос >= 0 Тогда
		...
		ПрочитатьСтруктуруЗапроса(стрТекстЗапроса);
	КонецЕсли;
...
Процедура ПакетЗапросов_Загрузить(ДополнительныеПараметры)
	...
	Оповещение = Новый ОписаниеОповещения("ПакетЗапросов_ЗагрузитьЗавершение", ЭтотОбъект, ДополнительныеПараметры, "ПакетЗапросов_ЗагрузитьОшибка", ЭтотОбъект);
...
&НаКлиенте
Процедура СкрытьПакетЗапросов(Команда)
	Элементы.ГруппаСписокЗапросов.Видимость = Не Элементы.ГруппаСписокЗапросов.Видимость;
	Если Элементы.ГруппаСписокЗапросов.Видимость Тогда
		Элементы.СкрытьПакетЗапросов.Картинка  = БиблиотекаКартинок.СкрытьПароль;
	Иначе
		Элементы.СкрытьПакетЗапросов.Картинка  = БиблиотекаКартинок.ПоказатьПароль;
	КонецЕсли;
КонецПроцедуры
