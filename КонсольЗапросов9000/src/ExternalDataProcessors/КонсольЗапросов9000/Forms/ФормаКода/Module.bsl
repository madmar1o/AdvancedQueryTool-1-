...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Перем ПараметрыОбъекта, ТекстЗапросаИзПараметра;
	
	Параметры.Свойство("Объект", ПараметрыОбъекта);
	Параметры.Свойство("Заголовок", Заголовок); 
	Параметры.Свойство("ИмяЗапроса", ИмяЗапроса); 
	Параметры.Свойство("ПараметрыЗапроса", ПараметрыЗапроса); 
	Параметры.Свойство("ТекстЗапроса", ТекстЗапросаИзПараметра);
	
	Если ЗначениеЗаполнено(ТекстЗапросаИзПараметра) Тогда
		ТекстЗапроса.УстановитьТекст(ТекстЗапросаИзПараметра);
	КонецЕсли;
	
	КопироватьДанныеФормы(ПараметрыОбъекта, Объект);
...
Функция ПолучитьЛитералЗначения(Значение, ИмяПараметра = Неопределено)
	...
	ИначеЕсли ТипЗначения = Тип("Дата") Тогда
		...
		КодЗначения = Формат(Год, "ЧЦ=4; ЧВН=; ЧГ=") + Формат(Месяц, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(День, "ЧЦ=2; ЧВН=; ЧГ=");
		Если Час <> 0 ИЛИ Мин <> 0 И Сек <> 0 Тогда
			КодЗначения = КодЗначения + Формат(Час, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Мин, "ЧЦ=2; ЧВН=; ЧГ=") + Формат(Сек, "ЧЦ=2; ЧВН=; ЧГ=");
		КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Тип("Граница") Тогда
		ЛитералЗначенияГраницы = ПолучитьЛитералЗначения(Значение.Значение);
		Литерал = СтрШаблон("Новый Граница(%1, %2)",
			ЛитералЗначенияГраницы.Литерал,
			СтрШаблон("ВидГраницы.%1", Значение.ВидГраницы));
	ИначеЕсли ТипЗначения = Тип("МоментВремени") Тогда
		ЛитералДаты = ПолучитьЛитералЗначения(Значение.Дата);
		ЛитералСсылки = ПолучитьЛитералЗначения(Значение.Ссылка);
		Литерал = СтрШаблон("Новый МоментВремени(%1, %2)",
			ЛитералДаты.Литерал, ЛитералСсылки.Литерал);
			Комментарий = ЛитералСсылки.Комментарий;
...
Процедура СоздатьКодСПараметрами()
	...
	ПараметрыЗапросаТаблицаЗначений = РеквизитФормыВЗначение("Объект").СтрокаВЗначение(ПараметрыЗапроса);
	
	Для Каждого СтрокаПараметра Из ПараметрыЗапросаТаблицаЗначений Цикл
