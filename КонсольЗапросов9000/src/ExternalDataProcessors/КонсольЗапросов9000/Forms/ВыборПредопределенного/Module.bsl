...
Процедура КомандаВставитьЗНАЧЕНИЕ(Команда)
	...
	Иначе
		НовыйЭлементы = ПолучитьИмя(Элемент);
		Закрыть(Новый Структура("ДанныеФормы, Результат", ДанныеФормы, "ЗНАЧЕНИЕ(" + ТипДляЗапроса(ТипОбъекта) + "." + ИмяОбъекта + "." + НовыйЭлементы + ")"));
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ПолучитьИмя(Знач ПеременнаяМетоданных) 
	Если ТипОбъекта = "Перечисления" Тогда
		ИмяМетаданных = ПеременнаяМетоданных.Метаданные().Имя;
		НазваниеМенеджера = СтрШаблон("%1Менеджер.%2", ТипДляЗапроса(ТипОбъекта), ИмяМетаданных); 
		Менеджер = Новый (НазваниеМенеджера);
		мИмя = Менеджер.Индекс(ПеременнаяМетоданных);
		Имя = ПеременнаяМетоданных.Метаданные().ЗначенияПеречисления[Менеджер.Индекс(ПеременнаяМетоданных)].Имя;
		Возврат Имя;
	ИначеЕсли ТипОбъекта = "Справочники" Тогда
		Возврат ПеременнаяМетоданных.ИмяПредопределенныхДанных;
	Иначе
		Возврат ПеременнаяМетоданных;
	КонецЕсли;
КонецФункции
